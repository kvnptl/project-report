...
\begin{document}
    ...
    % The Table \ref{table:quantitative_effect_of_weather} highlights the quantitative effect of various weather conditions on various sensors. It is worth noticing that level 3, moderate influences, that cause perception error up to 30\% of the time in this table, could also mean up to 30\% of the LiDAR point cloud is affected by noise, or up to 30\% of the pixels in the camera images are affected by distortion or obscure. The same applies to level 4 influences, serious, as well.
    ...
    % Lidar (a 850-950nm and 1550nm) & & & & & & & & \\
    % Radar (24, 77 and 122 GHz) & & & & & & & & \\
    % FIR Camera (a 800-950nm) & & & & & & & & \\
    % Thermal FIR Camera (a 2-10\textmu m) & & & & & & & & \\
    ...
    %     \end{table}
    % % \restoregeometry
    % The effect level of each phenomenon causes to sensors:
    % \begin{itemize}
    ...
    % \begin{table}[h]
    %     \centering
    %     \caption{The influence level of various weather conditions on sensors (NIR: Near-Infrared Camera, FIR: Far-Infrared Camera) \cite{zhang2023perception}}
    %     \begin{tabular}{|l|c|c|c|c|c|c|c|c|}
    %     \hline
    %     \textbf{Modality} & \textbf{Light rain} & \textbf{Heavy rain} & \textbf{Dense smoke} & \textbf{Fog} & \textbf{Haze} & \textbf{Snow} & \textbf{Strong light} & \textbf{Cost} \\
    %     & $<$4mm/hr & $<$25mm/hr & vis$<$0.1km & vis$<$0.5km & vis$<$2km & & (over emitter) & \\
    ...
    % This work underscores the importance of real-time strategy adaptation and smart sensor subset selection in maximizing the accuracy and dependability of multimodal perception systems.

    % However, Radecki et al.'s study \cite{radecki2016all} has certain limitations, 
    % - the omission of heavy traffic and urban environments. Additionally, 
    % - the study does not explore deep learning-based fusion techniques.
    % - limits the evaluation on limited weather samples on custom datasets, thereby limiting the applicability on the more real-world diverse weather conditions
    % - only explores early fusion technique by hand-tuned pre-processing of the sensor data which limits the generalization of the findings
